        ...
        <ul class="">
            <c:forEach items="${chatRooms}" var="chatRoom">
                <li class="flex mt-2">
                    ...
                    <a onclick="if ( !confirm('정말로 삭제하시겠습니까?') ) return false;" class="btn btn-outline btn-error btn btn-sm mr-2" href="/usr/article/delete/free/${chatRoom.id}?_method=DELETE">삭제</a>
                    <a class="btn btn-outline btn-primary btn btn-sm" href="/usr/article/modify/free/${chatRoom.id}">수정</a>
                </li>
            </c:forEach>
        </ul>
    </div>
